...
void main() {
  runZonedGuarded<Future<void>>(() async {
    ...
    // Custom Error Widget Builder to prevent red-screen crashes on Android UI render errors
    ...
    // Global Flutter framework error listener
    FlutterError.onError = (FlutterErrorDetails details) {
      FlutterError.presentError(details);
      debugPrint('Caught Flutter Error: ${details.exception}');
    };

    // Safe Supabase Initialization (Will fallback gracefully if offline/unreachable)
    try {
      ...
      debugPrint('Supabase Initialization Note (Using Fallback Feed): $e');
    }

    // Safe Google Mobile Ads Initialization (Protects against missing AndroidManifest App IDs)
    try {
    ...
    debugPrint('Async Uncaught Startup Error Handled: $error');
  });
}
